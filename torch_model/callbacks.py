...
class CheckPerformance(Callback):
    ...
    def on_epoch_end(self, logs=None):
        ...
        [metric1, metric2]= self.metric_func(self.trainer.model, self.data_loader, self.device)
        self.trainer.logs[self.name + " " + str(self.metrics).upper() + "1 "] = metric1
        self.trainer.logs[self.name + " " + str(self.metrics).upper() + "2 "] = metric2
        self.hist.append([metric1, metric2])
...
class EarlyStopping(Callback):
    def __init__(self, data=None, metrics="acc", patience=1, larger_is_better=True, eps=1e-12, save_path=None, only_save_best=True):
        ...
        self.count1 = 0
        self.count2 = 0
        self.best_metric1 = -math.inf if larger_is_better else math.inf
        self.best_metric2 = -math.inf if larger_is_better else math.inf
        if larger_is_better:
            self.compare = lambda a, b: a > b + eps    # 定义比较的方法
        else:
    ...
    def on_epoch_end(self, logs=None):
        ...
        [metric1, metric2] = self.metric_func(self.trainer.model, self.data_loader, self.device)
        self.hist.append([metric1, metric2])
        if self.compare(metric1, self.best_metric1):
            self.count1 = 0
        ...
        elif self.compare(metric2, self.best_metric2):
            self.count2 = 0
        ...
        if self.save_path is not None and not self.only_save_best:
            self.trainer.model.save(self.save_path)
        if self.count1 > self.patience and self.count2 > self.patience:
            self.trainer.end_training = True
        self.trainer.logs["Val " + str(self.metrics).upper() + "1"] = metric1
        self.trainer.logs["Best Val " + str(self.metrics).upper() + "1"] = self.best_metric1
        self.trainer.logs["Val " + str(self.metrics).upper() + "2"] = metric2
        self.trainer.logs["Best Val " + str(self.metrics).upper() + "2"] = self.best_metric2
